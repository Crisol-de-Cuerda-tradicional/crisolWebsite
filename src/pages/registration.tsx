...
export const getStaticProps: GetStaticProps = async ({ locale = "es" }) => {
  const registrationText =
    translations.registration[locale as Language].toUpperCase();
  const underageAuthorisationText =
    translations.underage_authorisation[locale as Language];
  const under14AuthorisationText =
    translations.under_14_authorisation[locale as Language];
  const acceptanceOfGuardianshipText =
    translations.acceptance_of_guardianship[locale as Language];
  ...
  const templateData = {
    ...
    endOfEarlyRegistrationDate: dayjs(
